...
(function() {
  ...
  FSCas = (function() {
    ...
    FSCas.prototype._getViaPath = function(pathsPath, cb) {
      var _this = this;
      if (this.cache.hasOwnProperty(pathsPath)) {
        return this._getViaHash(this.cache[pathsPath].hash, cb);
      } else {
    ...
    FSCas.prototype._storePathMapping = function(hash, destPath, cb) {
      ...
      return filelet.writeFile(normalizedPath, JSON.stringify(result, null, 2), 'utf8', function(err) {
        if (err) {
          return cb(err);
        } else {
          _this.cache[destPath] = result;
          return cb(null);
        }
      });
    };
